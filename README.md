...
数据保存在服务器的 SQLite 数据库里，是「个人使用界面」，目前没有登录验证 —— 部署到公网前请先看下面「关于管理后台的安全性」这一节。
...
`Menu/admin.html` 目前没有登录验证；如果把这个项目部署到公网，请至少把管理后台和写操作 API 放在内网/VPN 后面，或者加一层 Basic Auth / 密码中间件。
