...
Nocturne 是一份鸡尾酒酒单，也是一册属于深夜的私人饮用记录。前端仍然是没有构建步骤的原生 JavaScript，酒单数据、点单台账单和结账历史现在都保存在后端的数据库里 —— 运行和部署方式见项目根目录的 [`README.md`](../README.md)。
...
数据保存在服务器的 SQLite 数据库里，是「个人使用界面」，目前没有登录验证 —— 部署到公网前请先看项目根目录 README 里「关于管理后台的安全性」这一节。

## Project anatomy

```text
Bar menu/
├── backend/             # FastAPI + SQLite 后端（见根目录 README）
└── Menu/
...
前端本身仍然不需要构建步骤，但现在依赖 `backend/` 提供的 API 才能正常工作——直接双击打开 `index.html` 不会显示任何酒款，需要先启动后端服务（见根目录 README 的「本地运行」）。

## Updating the collection

酒款、配方、价格、风味描述现在都通过 [`admin.html`](./admin.html) 管理后台增删改，保存后客用酒单和点单台会在下次加载时自动读到最新数据，不需要再手改 `script.js` 或 `ledger.js`。

如果更喜欢直接改数据库，也可以调用后端 API（见根目录 README 的「API 一览」）。一款酒对应的字段是：英文名（唯一标识）、中文名、基酒、酒单分类（经典/特调）、价格、风味描述、配方（每行一种材料），以及可选的「点单台置顶顺序」。
